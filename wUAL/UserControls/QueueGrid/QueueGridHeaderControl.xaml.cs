...
namespace wUAL.UserControls
{
    ...
    public partial class QueueGridHeaderControl : UserControl
    {
        ...
        {
            InitializeComponent();
            if (MainApp.DesignMode && label==null && iconPath==null && icon==null)
                SetLabel("Testing", "Default");
            else
        ...
        {
            if (QueueIcon != null && !MainApp.DesignMode) {
                return;
            }
            if (QueueIconName == null || MainApp.DesignMode)
            {
