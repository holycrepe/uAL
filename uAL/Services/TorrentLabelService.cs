...
namespace uAL.Services
{
    ...
    public static class TorrentLabelService
    {
        ...
        static readonly ConcurrentQueue<string> History = new ConcurrentQueue<string>();

        public static TorrentLabel CreateSimpleTorrentLabel(string downloadDir, TorrentQueueItem queueItem)
            => CreateTorrentLabel(downloadDir, queueItem.File, false, queueItem);

        public static TorrentLabel CreateTorrentLabel(string downloadDir, TorrentQueueItem queueItem, bool extend = true,
                                                      bool? updateQueueItemLabel = null)
            => CreateTorrentLabel(downloadDir, queueItem.File, extend, queueItem,
                                      (updateQueueItemLabel.HasValue ? updateQueueItemLabel.Value : extend));

        public static TorrentLabel CreateTorrentLabel(string downloadDir, string newFilePath, bool extend = true,
                                                      TorrentQueueItem queueItem = null,
                                                      bool updateQueueItemLabel = true)
            => CreateTorrentLabel(downloadDir, FileUtils.GetInfo(newFilePath), extend, queueItem, updateQueueItemLabel);

        public static TorrentLabel CreateTorrentLabel(string downloadDir, FileInfo fi, bool extend = true,
                                                      TorrentQueueItem queueItem = null,
                                                      bool updateQueueItemLabel = true)
        {
            ...
            if (fi.Extension == ".torrent" && extend) {
                TorrentInfo tfi = (queueItem == null
                                       ? TorrentInfoCache.GetTorrentInfo(fi)
                ...
                if (tfi.success && !tfi.HasFileList) {
                    ...
                    var torrentContentsFileNameStripped =
                        Path.GetFileNameWithoutExtension(label.LargestName).StripFilename();
                    ...
                    if (failed) {
                        ...
                        label.UseExtendedName = LibSetting.Labels.ExtendByDefault;
                        if (!History.Contains(label.Base)) {
                            History.Enqueue(label.Base);
                            ...
                            var logEvent = new LogEventInfoCloneable(LogLevel.Info, Logger.Name, "Extended Label: ",
                                                                     eventInfo: eventInfo);
                            Logger.Log(logEvent);
                        }
                    }
