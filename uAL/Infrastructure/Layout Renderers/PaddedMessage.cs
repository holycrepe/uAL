...
namespace uAL.Infrastructure
{
    ...
    public class PaddedMessage : LayoutRenderer
    {
        ...
        void ReportErrorTitle(string title = "", string prefix = "  ", int pad = 0) { AppendLine(title, prefix, pad); }

        void ReportErrorText(string text = "", string prefix = "  ", int pad = 10) { AppendLine(text, prefix, pad); }
        ...
        protected override void Append(StringBuilder builder, LogEventInfo logEvent)
        {
            ...
            } else if (logEvent.Parameters != null && logEvent.Parameters.Count() == 1
                       && logEvent.Parameters[0].GetType().Equals(new Dictionary<string, object>())) {
            ...
            if (eventAttributes == null || logEvent.Message != logEvent.FormattedMessage) {
                ...
                string titleKey = (eventAttributes.ContainsKey("TitleValue")
                                       ? "TitleValue"
                                       : eventAttributes.ContainsKey("TitleSuffix") ? "TitleSuffix" : null);
