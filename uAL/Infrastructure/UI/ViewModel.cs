        ...
        where TValue : class
    {
        ...
        public ObservableCollection<T> SelectedItems
        {
            get
            {
                ...
                return this._selectedItems ?? (this._selectedItems = new ObservableCollection<T>());
            }
        }
        public ViewModel()
        {
            this.IsMultiple = this.IS_MULTIPLE_DEFAULT;
            Initialize();
        }
        ...
        {
            if (this.IgnoreSelectedItemsChanged)
                return;
        ...
        protected virtual void SetSelectedItems(TValue value)
        {
            if (!this.IsMultiple)
                return;
            using (this.IgnoreSelectedItemsChanged.On)
            {
                T matchingItem;
                if (GetMatchingItem(value, out matchingItem))
                {
                    this._selectedItem = matchingItem;
                    if (!this.SelectedItems.Contains(matchingItem))
                    {
                        this.SelectedItems.Clear();
                        this.SelectedItems.Add(matchingItem);
                        return;
                    }
                    for (var i = this.SelectedItems.Count - 1; i >= 0; i--)
                    {
                        if (this.SelectedItems[i] != matchingItem)
                        {
        ...
        #endregion
        #region Logging
        [System.Diagnostics.Conditional("LOG_PROPERTY_CHANGED")]
        [DebuggerNonUserCode]
