...
namespace Torrent.Queue
{
									 ...
									 IEquatable<int>, IEquatable<QueueStatusMember>, IEquatable<string>
	{
		...
		public QueueStatusMember(string name, QueueStatusMember previous, int offset = 1) : this(name, name, previous, offset)
		{
		...
		public QueueStatusMember(string name, string title, QueueStatusMember previous, int offset = 1) 
		{
			...
			this.Title = title;
			if (previous == null) {
				throw new ArgumentNullException(nameof(previous), $"{nameof(QueueStatusMember)}.ctor({nameof(name)}, {nameof(previous)}, {nameof(offset)})");
            }
		...
		{
			return other == this.Name || other == this.Title;
		}
		...
		static QueueStatusMember validStart => QueueStatus.Ready;
		...
		static QueueStatusRegion dupe { 
	 	...
	 	public QueueStatusMember SetReady() {
			return this == QueueStatus.Queued ? QueueStatus.Ready : this;
    	}
	 	bool isBetween(QueueStatusRegion region) {
			return region.Start <= this && this < region.End;
	 	}
