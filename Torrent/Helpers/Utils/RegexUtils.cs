...
namespace Torrent.Helpers.Utils
{
    public static partial class RegexUtils
    {
    	...
    	static string createGroup(IEnumerable<string> patterns) {
    		return $@"(?:{string.Join("|", patterns)})";
    	}
    	public static string CreateGroup(StringCollection patterns) {
        	return createGroup(patterns.Cast<string>());
    	}
    	public static string CreateGroup(IEnumerable<string> patterns) {
    		return createGroup(patterns);
    	}
    	public static string CreateGroup(params string[] patterns) {
    		return createGroup(patterns);
        }
        public static string[] GetDates(string subject)
        {
            if (dateRegexPattern == null)
            {
                ...
                dateRegexPattern = @"({Month}{Sep}{Day}{Sep}{Year}|{Year}{Sep}{Month}{Sep}{Day}|\b{Year}\b)".Format(Formats);
            }
