...
namespace Torrent.Helpers.Utils
{
    public static class FileSystemUtils
	{
		...
		public struct MoveFileResult {
			...
			public MoveFileResult(string newFileName, bool newlyAdded = false, MoveFileResultStatus status = MoveFileResultStatus.InProgress) {
				Status = status;
		...
		public static async Task<MoveFileResult> MoveAddedFile(FileInfo fi, string addedDir, TorrentLabel label = null, bool doMove = true, bool doLogInfo = false, string addedFileName = null, string addedLabel = null)
		{            
		...
		public static async Task<MoveFileResult> MoveFile(FileInfo fi, string newFileName, bool doLogInfo=true, string newFilenameSuffix = "", bool newlyAdded = false) {
			var filename = fi.FullName;
			...
			do {
				...
				try {
					...
					logger.Error(subjectEvent.Clone(ex, "File Access Error occurred while trying to move the " + extensionTitle));
				} catch (IOException ex) {
					if (ex.Message == "The process cannot access the file because it is being used by another process.") {	            		
						retry = true;						
					} else {
						var errorEvent = subjectEvent.Clone(ex, "IO Exception occurred while trying to move the " + extensionTitle).SetError();
						logger.Error(errorEvent);
					}	            	
				} catch (Exception ex) {
					logger.Error(subjectEvent.Clone(ex, "Unknown Exception occurred while trying to move the " + extensionTitle));
				}
				if (retry) {					
